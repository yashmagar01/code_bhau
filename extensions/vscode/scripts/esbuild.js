      ...
      setup(build) {
        build.onEnd((result) => {
          if (result.errors.length > 0) {
            ...
            try {
              // Code Bhau fork patch: ensure build/ exists before writing meta.json
              // (original assumed the directory was pre-created; in our forked
              // layout it isn't, which produced a noisy ENOENT in build output.)

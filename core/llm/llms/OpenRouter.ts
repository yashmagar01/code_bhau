...
// Code Bhau fork note:
// `OPENROUTER_HEADERS` was previously imported from "@continuedev/openai-adapters",
// but the constant is not exported by any published version of that package
// (latest as of fork: 1.37.0). It exists only in the upstream monorepo's
// local working copy. Defining it inline here is both the smallest possible
// patch AND the semantically correct one — OpenRouter's attribution headers
// should identify Code Bhau, not Continue.
// See: https://openrouter.ai/docs#request-headers
export const OPENROUTER_HEADERS: Record<string, string> = {
  "HTTP-Referer": "https://github.com/code-bhau/code-bhau-agent",
  "X-Title": "Code Bhau Agent",
};
